            ...
            if (sender is GridCommandCellElement)
            {
               switch ((((GridCommandCellElement)(sender)).CommandButton).TextElement.Text)
               {
                     ...
                     if (!String.IsNullOrEmpty(Presenter.TipoDocumentoVenta))
                     {
                        switch (Presenter.TipoDocumentoVenta)
                        {
                           ...
                           case "Factura":
                              ...
                              {
                                 case PRO007Presenter.TFacturacion.SLI:
                                    Presenter.AnularFactura_SLI();
                                    break;
                                 case PRO007Presenter.TFacturacion.Otros:
                                    Presenter.AnularFactura_OV();
                                    break;
                                 default:
                                    break;
                              }
                  ...
                  case "Facturar":
                     ...
                     break;
                  case "Impresión":
                     if (Infrastructure.WinForms.Controls.Dialogos.MostrarMensajePregunta(Presenter.Title, "¿Desea imprimir el Documento Seleccionado?", Infrastructure.WinForms.Controls.Dialogos.LabelBoton.Si_No) == System.Windows.Forms.DialogResult.Yes)
                     {
                        SeleccionarItemFactura();
                        ...
                        { Dialogos.MostrarMensajeInformacion(Presenter.Title, String.Format("La orden de venta: {0} se encuentra bloqueada, no podra imprimir ningun documento relacionado", Presenter.ItemDocsVta.OV_OP)); }
