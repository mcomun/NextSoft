...
{
   public partial class PRO007MView : Form, IPRO007MView
   {
      #region [ Variables ]
      ...
      private DialogResult LoadTarjas()
      {
         if (Delfin.Controls.AyudaTarjas.MostrarAyuda(Presenter.ContainerService, Presenter.ItemPreDocsVta.PDOC_Codigo, CbTIPO_CodMND.TiposSelectedValue, NUDPDOV_TipoCambio.Value) == DialogResult.OK)
         {
            btnBuscar.Enabled = false;
            Presenter.DTServicios = new DataTable();
            Presenter.DTServicios = Delfin.Controls.AyudaTarjas.RespuestaServicios;
            Presenter.GrillaItemsPreDetDocsVta = new ObservableCollection<PreDetDocsVta>();
      ...
      public void GetItemsDetalleServicios()
      {
         ...
         {
            item.DDOV_Impuesto1 = (Presenter.IGV / 100) * item.DDOV_ValorVenta;
      ...
      public void Calculos()
      {
         try
         {
            ...
            Decimal ddovImpuesto1 = (Presenter.IGV / 100) * ddovValorVenta;
      ...
      void btnBuscarTarja_Click(object sender, EventArgs e)
      {
         if (CbTIPO_CodMND.SelectedValue != null)
         {

            if (LoadTarjas() != DialogResult.OK) return;
            Presenter.ServiciosTarjas(true);
            ...
            CbTIPO_CodMND.TiposSelectedValue = Presenter.TipoMoneda ?? "001";
            if (Presenter.TipoFPago != null && Presenter.TipoFPago.Equals("001")) // Contado
            {
               CbTIPO_CodFPG.TiposSelectedValue = "001";
               DTPPDOV_FechaVcmto.NSFecha = Presenter.Session.Fecha;
            }
            else if (Presenter.TipoFPago != null && Presenter.TipoFPago.Equals("002")) // Credito
            {
               CbTIPO_CodFPG.TiposSelectedValue = "002";
               DTPPDOV_FechaVcmto.NSFecha = Presenter.DiasCredito > 0 ? DTPPDOV_FechaEmision.Value.Date.AddDays(Presenter.DiasCredito) : Presenter.Session.Fecha;
            }
            txaSeries.SetValue(!String.IsNullOrEmpty(Presenter.TipoDoc) ? Presenter.TipoDoc : "001", null);
            //CbTIPO_CodTDO.TiposSelectedValue = !String.IsNullOrEmpty(Presenter.TipoDoc) ? Presenter.TipoDoc : "001";
            //              NUDPDOV_TipoCambio.Value = Presenter.TipoCambio > 0 ? Presenter.TipoCambio : 0;
            CbTIPO_CodMND.Enabled = false;
         }
         else
