...
Delfin.Web.Std.Components.ProgressBar = function (opts) {
    this.init(opts);
};

Delfin.Web.Std.Components.ProgressBar.prototype = {
    idDivProgrees: 'divProgressDelfin',
    idDivDialog: 'divProgressDialogDelfin',
    ...
    _privateFunction: {
        ...
        implementProgressElement: function (targetLoading, maxValue) {
            ...
            if (divProgress.length == 0) {
                ...
                divProgress.addClass("progressBar-Delfin");
                divProgress.append($('<div class="progressBar-Delfin-label"></div>'));
                
                if (Delfin.Web.Std.Shared.General.Resources) {
                   divProgress.find('.progressBar-Delfin-label').text(Delfin.Web.Std.Shared.General.Resources.EtiquetaCargando);
                }
            ...
            var config = {
                ...
                change: function () {
                    if (me.getValue() != false) {
                        divProgress.find('.progressBar-Delfin-label').text(me.getValue() + "%");
                    }
                }
            };
