...
{
   public partial class MAN003LView : UserControl, IMAN003LView
   {
      #region [ Variables ]
      ...
      #region [ Formulario ]
      public MAN003LView(Telerik.WinControls.UI.RadPageViewPage x_tabPageControl)
      {
         InitializeComponent();
         try
         {
      ...
      private void SeleccionarItem()
      {
         try
         {
            if (Presenter != null)
            {
               if (BSItems != null && BSItems.Current != null && BSItems.Current is Entities.ResultGestion)
               { Presenter.Item = ((Entities.ResultGestion)BSItems.Current); }
               else
               { Presenter.Item = null; }
            }
         }
