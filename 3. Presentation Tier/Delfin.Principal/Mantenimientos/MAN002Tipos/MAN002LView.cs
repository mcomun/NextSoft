...
{
   public partial class MAN002LView : UserControl, IMAN002LView
   {
      #region [ Variables ]
      ...
      #region [ Formulario ]
      public MAN002LView(Telerik.WinControls.UI.RadPageViewPage x_tabPageControl)
      {
         InitializeComponent();
         try
         {
            ...
            cmbTIPO_CodTIP.SelectedIndexChanged += cmbTIPO_CodTIP_SelectedIndexChanged;
      ...
      public void LoadView()
      {
         try
         {
            ...
            cmbTIPO_CodTIP.LoadControl(Presenter.ContainerService, "Tipos de Tabla", "TIP", "< Seleccionar Tipos >", ListSortDirection.Ascending);
            ...
            btnMAN_Nuevo.Enabled = false;
            btnMAN_Buscar.Enabled = false;
      ...
      private void FormatDataGrid()
      {
         try
         {
            ...
            this.grdItems.Columns.Add("TIPO_CodTipo", "Código", "TIPO_CodTipo");
            this.grdItems.Columns.Add("TIPO_Desc1", "Descripción", "TIPO_Desc1");
            this.grdItems.Columns.Add("TIPO_DescC", "Descripción Corta", "TIPO_DescC");

            if (cmbTIPO_CodTIP.TiposSelectedValue == "PAI")
            { this.grdItems.Columns.Add("TIPO_DescTipoHijo", "Tráfico", "TIPO_DescTipoHijo"); }
            ...
            foreach (Telerik.WinControls.UI.GridViewDataColumn column in grdItems.Columns)
            {
               ...
               if (Presenter.ItemTabla != null && (Presenter.ItemTabla.TIPO_CodTipo.Equals("MND") || Presenter.ItemTabla.TIPO_CodTipo.Equals("MOV")))
               { grdItems.Columns["Delete"].IsVisible = false; }
            }
      ...
      private void SeleccionarTabla()
      {
         try
         {
            //if (cmbMAN.SelectedItem != null && cmbMAN.SelectedItem is Entities.Tipos)
            //{ Presenter.ItemTabla = (Entities.Tipos)cmbMAN.SelectedItem; }
            //else
            //{ Presenter.ItemTabla = null; }

            Presenter.ItemTabla = cmbTIPO_CodTIP.TiposSelectedItem;

            if (Presenter.ItemTabla != null)
            {
               if (Presenter.ItemTabla.TIPO_CodTipo.Equals("MND") || Presenter.ItemTabla.TIPO_CodTipo.Equals("MOV"))
               {
                  btnMAN_Nuevo.Enabled = false;
                  grdItems.Columns["Delete"].IsVisible = false;
               }
               ...
               {
                  grdItems.Columns["Delete"].IsVisible = true;
                  btnMAN_Nuevo.Enabled = (Presenter.ItemTabla != null);
                  btnMAN_Buscar.Enabled = (Presenter.ItemTabla != null);
               }
            }
            else
            {
         ...
         { Infrastructure.WinForms.Controls.Dialogos.MostrarMensajeError(Presenter.Title, "Ha ocurrido un error al seleccionar la tabla.", ex); }
      }
      private void SeleccionarItem()
      {
         try
         {
            if (Presenter != null)
            {
               if (BSItems != null && BSItems.Current != null && BSItems.Current is Entities.Tipos)
               { Presenter.Item = ((Entities.Tipos)BSItems.Current); }
               else
               { Presenter.Item = null; }
            }
         }
      ...
      private void Deshacer()
      {
         ...
         {
            //cmbMAN.SelectedIndex = -1;
            cmbTIPO_CodTIP.TiposSelectedValue = null;
      ...
      //{ SeleccionarTabla(); }
      private void cmbTIPO_CodTIP_SelectedIndexChanged(object sender, EventArgs e)
      { SeleccionarTabla(); FormatDataGrid(); }
