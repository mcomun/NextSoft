...
using Delfin.Principal.Procesos.Facturacion;
...
namespace Delfin.Principal
{
   public class PRO007Presenter
   {
      ...
      public void AnularFactura_SLI(string TipoDocumentoVenta)
      {
         try
         {
            if (ItemDocsVta != null)
            {
               ...
               if (result == DialogResult.Yes)
               {
                  ...
                  if (Client.AnularDocsVtaFacturacion(ItemDocsVta, TipoDocumentoVenta))
                  {
                     Dialogos.MostrarMensajeSatisfactorio(Title, "Se ha anulado el item.");
                     Actualizar();
                  }
      ...
      public void AnularFactura_OV(string TipoDocumentoVenta)
      {
         try
         {
            if (ItemDocsVta != null)
            {
               ...
               if (result == DialogResult.Yes)
               {
                  ...
                  if (TipoDocumentoVenta == "eFact")
                  { 
                  if (Client.SaveDocsVta(ref _item, DocsVta.TInterfazDocsVta.AnularFacturaFE))
                  {
                     Dialogos.MostrarMensajeSatisfactorio(Title, "Se ha anulado el item.");
                     Actualizar();
                  }
      ...
      public bool Guardar()
      {
         try
         {
             if (TipoDocumentoVenta.Equals("Factura") || TipoDocumentoVenta.Equals("eFact"))
            {
      ...
      public void EmisionFA()
      {
              ...
              if (DSReporte != null)
              {
                  ...
                  switch (TImpresion)
                  {
                      case TipoImpresion.Imprimir:
                          ...
                          {
                              if (DSReporte.Tables[0].Rows[0]["UnidadNegocio"].ToString() != "S")
                              {
                          ...
                          else
                          {
                              ...
                              {
                                  _para.PARA_Valor = "";
                                  if (!DSReporte.Tables[0].Rows[0]["DOCV_Serie"].Equals("009"))
                                  {
       ...
       public void EmisionFE()
       {
           try
         {
            ...
            if (ItemDocsVta.DOCV_Numero !=null)
            {
                String Directorio = "";
                string RucNo = Directorio = ConfigurationManager.AppSettings["RUC"]; ;
                String TipoDocumento = ItemDocsVta.TIPO_CodTDO;

                switch (TipoDocumento)
                {
                    case "001":
                        Directorio = ConfigurationManager.AppSettings["pathpdfFacturas"];
                        break;
                    case "003":
                        Directorio = ConfigurationManager.AppSettings["pathpdfBoletas"];
                        break;
                    case "007":
                        Directorio = ConfigurationManager.AppSettings["pathpdfNotasCredito"];
                        break;
                }

                String[] files = System.IO.Directory.GetFiles(Directorio, ItemDocsVta.DOCV_Serie + "-" + ItemDocsVta.DOCV_Numero + "*.pdf");
                if (files.Length == 0)
                { files = System.IO.Directory.GetFiles(Directorio, RucNo + "-" + TipoDocumento.Substring(1, 2) + "-" + ItemDocsVta.DOCV_Serie + "-" + ItemDocsVta.DOCV_Numero + "*.pdf"); }
                ...
                if (archivopdf != "")
                {
                    ...
                    PRO007RViewFE popupPRO007RViewFE = new PRO007RViewFE();
                    popupPRO007RViewFE.pathpdf = archivopdf;
                    dr = popupPRO007RViewFE.ShowDialog();

                }
                else
                {
                    Dialogos.MostrarMensajeError("Facturacion Electronica", "No existe factura electronica", false);
                }
               return;
           ...
           //Validar Email
                ...
                if (ItemDocsVta.emailFE == null || ItemDocsVta.emailFE.Trim() == "")
	            {
                    DialogResult dr = new DialogResult();
                    PRO007DViewImprimirFE popupPRO007DViewImprimirFE = new PRO007DViewImprimirFE();
                    popupPRO007DViewImprimirFE.entidad = Entc_Codigo;

                    dr =  popupPRO007DViewImprimirFE.ShowDialog();
                    if ( dr == DialogResult.OK ) 
                    {
                         email = popupPRO007DViewImprimirFE.email;
                         Imprimir = popupPRO007DViewImprimirFE.Imprimir;
                    }
            ...
            _listFilters.Add(new Infrastructure.Aspect.DataAccess.DataAccessFilterSQL() { FilterName = "@pvchAUDI_UsrMod", FilterValue = Session.UserName, FilterType = Infrastructure.Aspect.DataAccess.DataAccessFilterTypes.Varchar, FilterSize = 20 });
            _listFilters.Add(new Infrastructure.Aspect.DataAccess.DataAccessFilterSQL() { FilterName = "@Serie", FilterValue = ItemDocsVta.DOCV_Serie, FilterType = Infrastructure.Aspect.DataAccess.DataAccessFilterTypes.Varchar, FilterSize = 20 });
            DSReporte = Client.GetImpresionFEDSDocsVta("VEN_DDOVSS_TodosImpresionByDOCV_Codigo", _listFilters, Session.UserName, Entorno.ItemSucursal.SUCR_Codigo, true, email);   //Client.GetAllDocsVtaImpresionFactura(ItemDocsVta.DOCV_Codigo);

            if (DSReporte != null)
                {

                    if (DSReporte.Tables[2] !=null)
                    {

                        DataTable dtFacturacionElectronica = new DataTable();
                        dtFacturacionElectronica = DSReporte.Tables[2];
                        string pathpdf = dtFacturacionElectronica.Rows[0]["pathpdf"].ToString();
                        string resultado = dtFacturacionElectronica.Rows[0]["resultado"].ToString();
                        string mensajeerror = dtFacturacionElectronica.Rows[0]["mensajeerror"].ToString();
                        ...
                        if (resultado=="OK")
                        {
                            ...
                            DialogResult dr = new DialogResult();
                            PRO007RViewFE popupPRO007RViewFE = new PRO007RViewFE();
                            popupPRO007RViewFE.pathpdf = pathpdf;
                            dr = popupPRO007RViewFE.ShowDialog();
                        ...
                        {
                            Dialogos.MostrarMensajeError("Facturacion Electronica", mensajeerror, false); 
                        }
          ...
          try
          {
              ...
              if (ItemDocsVta != null && ItemDocsVta.DOCV_Codigo != 0)
              {
                  ...
                  if (ItemDocsVta.DOCV_Numero != null)
                  {
                      ...
                      // ConfigurationManager.AppSettings["pathpdfFacturas"];
                      //String[] files = System.IO.Directory.GetFiles(Directorio, strFechaReporte & "*.xlsx");
                      return;
                  ...
                  if (ItemDocsVta.emailFE == null || ItemDocsVta.emailFE == "")
                  {
                      DialogResult dr = new DialogResult();
                      PRO007DViewImprimirFE popupPRO007DViewImprimirFE = new PRO007DViewImprimirFE();
                      popupPRO007DViewImprimirFE.entidad = Entc_Codigo;
                  ...
                  if (DSReporte != null)
                  {
                      int totalTablas = DSReporte.Tables.Count;
                      if (DSReporte.Tables[totalTablas - 1] != null)
                      {

                          DataTable dtFacturacionElectronica = new DataTable();
                          dtFacturacionElectronica = DSReporte.Tables[totalTablas - 1];
                          string pathpdf = dtFacturacionElectronica.Rows[0]["pathpdf"].ToString();
                          string resultado = dtFacturacionElectronica.Rows[0]["resultado"].ToString();
                          string mensajeerror = dtFacturacionElectronica.Rows[0]["mensajeerror"].ToString();
      ...
      public void ImpresionRC()
      {
         try
         {
            ...
            {
                DSReporte = Client.GetDSDocsVta("CAJ_CCCTSS_ImprimirNuevoReciboCaja", _listFilters, true);
                //Validar Email
                email = ItemDocsVta.emailFE;
                if (ItemDocsVta.emailFE == null || ItemDocsVta.emailFE == "")
                {
                    DialogResult dr = new DialogResult();
                    PRO007DViewImprimirFE popupPRO007DViewImprimirFE = new PRO007DViewImprimirFE();
                ...
                if (DSReporte != null)
                {
                    PRO007RViewRC oForm = new PRO007RViewRC();
                    oForm.dtPrint = DSReporte.Tables[0];
                    oForm.aParams = GetParamValues();
                    oForm.eMail = email;
                    oForm.RptFile = "ReciboCaja.rpt";
                    oForm.ShowDialog();
                }
            }
      ...
      internal ArrayList GetParamValues()
      {
          ArrayList aParams = new ArrayList();
          string Auditoria = "Usuario: " + Session.UserName + "   Fecha: " + DateTime.Now.ToString();
          aParams.Add("");
          aParams.Add("");
                     ...
                     case 0: // Logistico 
                        if (DTServicios.Select("CTAR_Tipo = 'L'").Any())
                        {
                           ...
                           ItemPreDetDocsVta = new PreDetDocsVta
                           {
                              ...
                              DDOV_Cantidad = 1,
                              DDOV_Impuesto1 = (decimal)DTServicios.Compute("SUM(Impuesto)","CTAR_Tipo = 'L'"), //DTServicios.Select("CTAR_Tipo = 'L'").Sum(e => (Decimal)e.ItemArray[21]),
                              DDOV_ValorTotal = (decimal)DTServicios.Compute("SUM(TotalVenta)","CTAR_Tipo = 'L'")
                           };
                        ...
                        if (DTServicios.Select("ISNULL(CTAR_Tipo, '') = ''").Any()) // devuelve si hay sin servicios adicionales
                        {
                           foreach (DataRow dr in DTServicios.Select("ISNULL(CTAR_Tipo, '') = ''"))
                           {
                              ...
                              ItemPreDetDocsVta = new PreDetDocsVta
                              {
                                 ...
                                 DDOV_Cantidad = 1,
                                 DDOV_Impuesto1 = (decimal)dr["Impuesto"],
                                 DDOV_ValorTotal = (decimal)dr["TotalVenta"]
                              };
                              GrillaItemsPreDetDocsVta.Add(ItemPreDetDocsVta);
                              ItemsPreDetDocsVta_Det_Operacion.Add(ItemPreDetDocsVta_Det_Operacion);
                           }
                            ...
                            foreach (var dr in DTServicios.Select("CTAR_Tipo = 'L'"))
                            {
                                ...
                                if (orDefault != null)
                                    ItemPreDetDocsVta = new PreDetDocsVta
                                    {
                                        ...
                                        DDOV_Impuesto1 = DTServicios.Select("CTAR_Tipo = 'L'").Sum(e => (Decimal)e.ItemArray[21]),
                                        DDOV_ValorTotal = DTServicios.Select("CTAR_Tipo = 'L'").Sum(e => (Decimal)e.ItemArray[22])
                                    };
                            //}
                           GrillaItemsPreDetDocsVta.Add(ItemPreDetDocsVta);
                        }
                           ...
                           foreach (DataRow dr in DTServicios.Select("ISNULL(CTAR_Tipo, '') = ''"))
                           {
                              ...
                              if (orDefault != null)
                                 ItemPreDetDocsVta = new PreDetDocsVta
                                 {
                                    ...
                                    DOCV_Codigo = Int32.Parse(orDefault.ItemArray[9].ToString()),
                                    PDDO_Item = Int16.Parse(orDefault.ItemArray[10].ToString()),
                                    DDOV_Impuesto1 = (decimal)dr["DDOV_Impuesto1"],
                                    DDOV_ValorTotal = (decimal)dr["DDOV_ValorTotal"]
                                 };
