...
{
   public partial class OPE009Presenter
   {
      #region [ Variables ]
      public String Title = "Reportes Gerencia";
      public String CUS = "OPE009";
      #endregion
      
      #region [ Contrusctor ]
      public OPE009Presenter(IUnityContainer x_container, IOPE009View x_mview)
      {
         try
         {
      ...
      public IOPE009View MView { get; set; }
      ...
      //}
      public void Cargar(ObservableCollection<Infrastructure.Aspect.DataAccess.DataAccessFilterSQL> x_filters)
      {
         try
         {
            ...
            //if (_itemTipoCambio != null)
            //{
            System.Data.DataTable dtStatment = Client.OPE_GetCab_Cotizacion_OVStatmentChange("COM_PREFSS_GenerarStatment_Change", x_filters);

            if (dtStatment != null && dtStatment.Rows.Count > 0)
            { Infrastructure.WinForms.Controls.ExportarExcel.Export(dtStatment); }
            else
            { Infrastructure.WinForms.Controls.Dialogos.MostrarMensajeInformacion(Title, "No se han encontrado registros para emitir el Statment de Change Control"); }
            //}
            //else
            //{ Infrastructure.WinForms.Controls.Dialogos.MostrarMensajeInformacion(Title, "Debe ingresar el Tipo de Cambio para la fecha " + Session.Fecha.ToShortDateString()); }
         }
