...
{
   public partial class PRO006LView : UserControl, IPRO006LView
   {
      #region [ Variables ]
      ...
      #region [ Formulario ]
      public PRO006LView(RadPageViewPage x_tabPageControl)
      {
         InitializeComponent();
         try
         {
      ...
      public void ShowItems()
      {
         try
         {
            BSItems.DataSource = Presenter.ItemsCab_Seguimientos;
            grdItems.DataSource = BSItems;
            navItems.BindingSource = BSItems;
            BSItems.ResetBindings(true);
      ...
      public void FiltrosLView()
      {
          try
          {
              ...
              Presenter.FILTRONave = String.IsNullOrEmpty(txtNave.Text) ? txtNave.Text : null;
          }
          catch (Exception ex)
          { Dialogos.MostrarMensajeError(Presenter.Title, Mensajes.LoadView, ex); }
      }
      ...
      private void FormatDataGrid()
      {
         try
         {
            ...
            grdItems.Columns.Add("CSEG_HBL", "HBL", "CSEG_HBL");
            grdItems.Columns.Add("CSEG_FechaEmision", "Fecha Emisión", "CSEG_FechaEmision");
      ...
      private void SeleccionarItem()
      {
         try
         {
            if (Presenter != null)
            {
                if (BSItems != null && BSItems.Current != null && BSItems.Current is Cab_Seguimientos)
               { Presenter.ItemCab_Seguimientos = ((Cab_Seguimientos)BSItems.Current); }
               else
               { Presenter.ItemCab_Seguimientos = null; }
            }
         }
         catch (Exception ex)
         {
