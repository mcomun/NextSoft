...
{
   public class COM005Presenter
   {
      #region [ Variables ]
      public String Title = "KPI Número de Reuniones";
      public String CUS = "COM005";
      #endregion

      #region [ Contrusctor ]
      public COM005Presenter(IUnityContainer x_container, ICOM005LView x_lview)
      {
         try
         {
      ...
      public ICOM005LView LView { get; set; }
         ...
         try
         {
            if (Desde.HasValue)
            {
               if (Hasta.HasValue)
               {
                  ...
                  {
                     if (Ejecutivo != null)
                     { DTKPIs = Client.GetGestionVentasKPIReuniones(Entorno.ItemEmpresa.EMPR_Codigo, Entorno.ItemSucursal.SUCR_Codigo, Desde.Value, Hasta.Value, PorEjecutivo, Ejecutivo.ENTC_Codigo); }
                     else
                     { Infrastructure.WinForms.Controls.Dialogos.MostrarMensajeInformacion(Title, "Debe seleccionar un Ejecutivo."); }
                  }
                  else
                  { DTKPIs = Client.GetGestionVentasKPIReuniones(Entorno.ItemEmpresa.EMPR_Codigo, Entorno.ItemSucursal.SUCR_Codigo, Desde.Value, Hasta.Value, PorEjecutivo, null); }

                  if (DTKPIs != null && DTKPIs.Rows.Count > 0)
                  {
                     if (!PorEjecutivo)
                     {
                        ReportPath = System.Windows.Forms.Application.StartupPath + @"\Reportes\rptKPI002Reuniones.rdlc";
                     ...
                     {
                        ReportPath = System.Windows.Forms.Application.StartupPath + @"\Reportes\rptKPI002ReunionesEjecutivo.rdlc";
                        RepDataSource = new Microsoft.Reporting.WinForms.ReportDataSource();
                        RepDataSource.Name = "DTKPIs";
                        RepDataSource.Value = DTKPIs;
