...
{
   public class COM011Presenter
   {
      #region [ Variables ]
      public String Title = "KPI TEUS Vendidos";
      public String CUS = "COM011";
      #endregion

      #region [ Contrusctor ]
      public COM011Presenter(IUnityContainer x_container, ICOM011LView x_lview)
      {
         try
         {
      ...
      public ICOM011LView LView { get; set; }
         ...
         try
         {
            if (Desde.HasValue)
            {
               if (Hasta.HasValue)
               {
                  ...
                  {
                     if (Ejecutivo != null)
                     { DTKPIs = Client.GetAllCab_Cotizacion_OVKPITEUSVendidos(Entorno.ItemEmpresa.EMPR_Codigo, Entorno.ItemSucursal.SUCR_Codigo, Desde.Value, Hasta.Value, PorEjecutivo, Ejecutivo.ENTC_Codigo); }
                     else
                     { Infrastructure.WinForms.Controls.Dialogos.MostrarMensajeInformacion(Title, "Debe seleccionar un Ejecutivo."); }
                  }
                  else
                  { DTKPIs = Client.GetAllCab_Cotizacion_OVKPITEUSVendidos(Entorno.ItemEmpresa.EMPR_Codigo, Entorno.ItemSucursal.SUCR_Codigo, Desde.Value, Hasta.Value, PorEjecutivo, null); }

                  if (DTKPIs != null && DTKPIs.Rows.Count > 0)
                  {
                     if (!PorEjecutivo)
                     {
                        ReportPath = System.Windows.Forms.Application.StartupPath + @"\Reportes\rptKPI005TEUSVendidos.rdlc";
                     ...
                     {
                        ReportPath = System.Windows.Forms.Application.StartupPath + @"\Reportes\rptKPI005TEUSVendidosEjecutivo.rdlc";
                        RepDataSource = new Microsoft.Reporting.WinForms.ReportDataSource();
                        RepDataSource.Name = "DTKPIs";
                        RepDataSource.Value = DTKPIs;
