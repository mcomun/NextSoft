...
{
   public class COM010Presenter
   {
      #region [ Variables ]
      public String Title = "KPI Ordenes de Venta";
      public String CUS = "COM010";
      #endregion

      #region [ Contrusctor ]
      public COM010Presenter(IUnityContainer x_container, ICOM010LView x_lview)
      {
         try
         {
      ...
      public ICOM010LView LView { get; set; }
         ...
         try
         {
            if (Desde.HasValue)
            {
               if (Hasta.HasValue)
               {
                  ...
                  {
                     if (Ejecutivo != null)
                     { DTKPIs = Client.GetAllCab_Cotizacion_OVKPIOrdenesVenta(Entorno.ItemEmpresa.EMPR_Codigo, Entorno.ItemSucursal.SUCR_Codigo, Desde.Value, Hasta.Value, PorEjecutivo, Ejecutivo.ENTC_Codigo); }
                     else
                     { Infrastructure.WinForms.Controls.Dialogos.MostrarMensajeInformacion(Title, "Debe seleccionar un Ejecutivo."); }
                  }
                  else
                  { DTKPIs = Client.GetAllCab_Cotizacion_OVKPIOrdenesVenta(Entorno.ItemEmpresa.EMPR_Codigo, Entorno.ItemSucursal.SUCR_Codigo, Desde.Value, Hasta.Value, PorEjecutivo, null); }

                  if (DTKPIs != null && DTKPIs.Rows.Count > 0)
                  {
                     if (!PorEjecutivo)
                     {
                        ReportPath = System.Windows.Forms.Application.StartupPath + @"\Reportes\rptKPI004OrdenesVenta.rdlc";
                     ...
                     {
                        ReportPath = System.Windows.Forms.Application.StartupPath + @"\Reportes\rptKPI004OrdenesVentaEjecutivo.rdlc";
                        RepDataSource = new Microsoft.Reporting.WinForms.ReportDataSource();
                        RepDataSource.Name = "DTKPIs";
                        RepDataSource.Value = DTKPIs;
