...
// Assembly location: C:\Users\ferar\Documents\My Solutions\NextSoft\Librerias\Delfin.Controls.dll
      ...
      try
      {
        ...
        using (IEnumerator<Entidad> enumerator = _olBCCRecipients.GetEnumerator())
        {
          while (((IEnumerator) enumerator).MoveNext())
          {
            ...
            {
              flag = false;
              x_detalle = x_detalle + "* " + current.get_ENTC_NomCompleto() + Environment.NewLine;
            }
            else
            {
        ...
        if (!flag)
        {
          int num = (int) Dialogos.MostrarMensajeInformacion("Entidades sin Email", "A las siguientes Entidad no fueron incluidos para el envio de este correo por no tener un Email (Ver Detalles).", x_detalle);
        }
        // ISSUE: reference to a compiler-generated method
        // ISSUE: variable of a compiler-generated type
